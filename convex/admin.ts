...
export const overview = query({
  ...
  handler: async ctx => {
    ...
    const users = (await ctx.db.query('profiles').collect()).sort((a, b) => a._creationTime - b._creationTime)
    const leaguesRaw = (await ctx.db.query('leagues').collect()).sort((a, b) => b._creationTime - a._creationTime)
    ...
    const courses = (await ctx.db.query('courses').collect()).sort((a, b) => a.name.localeCompare(b.name))
