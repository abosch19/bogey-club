    ...
    <div className="space-y-1.5">
      {rankings.map((p, i) => {
        ...
        return (
          ...
          >
            <div
              className="w-7 h-7 rounded-[7px] flex items-center justify-center font-mono font-black text-[13px]"
            ...
            <div
              ...
              style={{ backgroundColor: GROUP_COLORS[p.group - 1] }}
            />
    ...
    <div className="space-y-3">
      ...
      <div className="flex gap-1.5">
        {Array.from({ length: nGroups }, (_, i) => i + 1).map(g => {
          ...
          const maxHole = gPlayers.length > 0 ? Math.max(...gPlayers.map(p => (scores[p.id] ?? []).length)) : 0
          return (
            <button
              type="button"
              key={g}
              ...
              style={{
                backgroundColor: activeGroup === g ? GROUP_COLORS[g - 1] : '#fff',
                color: activeGroup === g ? '#fff' : '#6b7a72',
                border: `1.5px solid ${activeGroup === g ? GROUP_COLORS[g - 1] : '#e5e0d4'}`,
              }}
            >
              <span>Grupo {g}</span>
              {maxHole > 0 && (
                <span className="font-mono text-[8px] opacity-80">
                  H{maxHole}/{totalHoles}
                </span>
              )}
            </button>
          )
  ...
  return (
    <div className="min-h-screen bg-[#f4f1e9] pb-8">
      <div className="safe-top px-[14px] pt-3 pb-4">
            ...
            <div className="flex items-center gap-2 mt-2">
              ...
              {myGroup && (
                <span
                  ...
                  style={{ backgroundColor: GROUP_COLORS[myGroup.group - 1], color: '#fff' }}
                >
        ...
        {tab === 'leaderboard' && <LeaderboardTab rankings={rankings} myId={myId} mode={mode} coursePar={coursePar} />}
