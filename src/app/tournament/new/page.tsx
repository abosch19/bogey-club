...
function PlayerPicker({ allProfiles, selected, search, groupSize, setConfig, onToggle }: PlayerPickerProps) {
  return (
    <div>
          ...
          return (
            ...
            >
              ...
              <div className="flex-1 text-left">
                ...
                <p className="font-mono text-[10px]" style={{ color: isSel ? 'rgba(255,255,255,0.5)' : '#6b7a72' }}>
...
function ConfigStep({ config, setConfig, courses, allProfiles, selected, onTogglePlayer, onNext }: ConfigStepProps) {
            ...
            {MODES.map(m => (
              ...
              >
                <div>
                  ...
                  <p className="text-[11px]" style={{ color: mode === m.id ? 'rgba(255,255,255,0.55)' : '#6b7a72' }}>
                    {m.desc}
                  </p>
                </div>
                {mode === m.id && (
...
type GruposStepProps = {
  groups: Player[]
  nGroups: number
  selected: Player[]
  groupSize: number
  saving: boolean
  setGroups: (groups: Player[]) => void
  onMoveToGroup: (playerId: Id<'profiles'>, newGroup: number) => void
  onCreate: () => void
}
...
function NuevoTorneoWizard({
  ...
  const { name, courseId, mode, groupSize } = config
