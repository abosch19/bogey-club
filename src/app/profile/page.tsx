...
function Sparkline({ diffs }: { diffs: { diff: number }[] }) {
  ...
  const w = 300,
    ...
    pad = 6
  const pts = vals
    .map((v, i) => `${pad + (i / (vals.length - 1)) * (w - pad * 2)},${pad + ((max - v) / range) * (h - pad * 2)}`)
    .join(' ')
  return (
...
export default function PerfilPage() {
  ...
  async function handleDeleteProfile() {
    if (
      !confirm('¿Borrar tu perfil? Se eliminarán tus rondas, golpes y estadísticas. Esta acción no se puede deshacer.')
    )
      return
    try {
      await deleteMe({})
