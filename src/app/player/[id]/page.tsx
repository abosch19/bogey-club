  ...
  if (player === null) {
    return (
      <div className="min-h-screen bg-[#f4f1e9] flex flex-col items-center justify-center gap-3">
        ...
        <Link to="/players" className="text-[#1f8a5b] font-semibold text-[13px]">Ver jugadores →</Link>
      </div>
    )
  }
        ...
        <HeroCard className="p-5 mb-3">
          <div>
            ...
            <div className="flex items-center gap-4 mb-5">
              <Avatar name={player.name} src={player.avatar_url} size={56}
              ...
              <div>
                <div className="flex items-center gap-2">
                  ...
                  {player.clubs_sponsor_url && (
                    ...
                    <img src={player.clubs_sponsor_url} alt="Sponsor de palos"
                      className="h-[13px] w-auto opacity-80" style={{ filter: 'brightness(0) invert(1)' }} />
                  )}
                </div>
                <p className="text-white/45 text-[12px] mt-0.5">{player.rounds_count} ronda{player.rounds_count !== 1 ? 's' : ''} jugada{player.rounds_count !== 1 ? 's' : ''}</p>
              </div>
            </div>
            <div className="flex items-end justify-between border-t border-white/10 pt-3.5">
              ...
              <div className="text-center">
                ...
                <p className="text-white text-[28px] font-black leading-none">{formatHandicap(player.handicap_index_pp)}</p>
              </div>
            ...
            {player.rounds.map(r => (
              <Link key={r.id} to={`/scorecard?round=${r.id}`}
                ...
                <div className="text-right">
                  ...
                  <p className="font-mono text-[10px] font-bold" style={{ color: r.delta <= 0 ? '#1f8a5b' : '#9b6e1a' }}>
