...
function Sparkline({ values, color = '#1f8a5b' }: { values: number[]; color?: string }) {
  ...
  const pts = values.map(
    (v, i) => [pad + (i / (values.length - 1)) * (w - pad * 2), pad + ((max - v) / range) * (h - pad * 2)] as const,
  )
...
export default function StatsPage() {
      ...
      return [
        {
          ...
          gir: myS.filter(s => s.gir).length,
          gir_total: myS.length,
  ...
  const hcpIndex: number | null =
    courseType === 'pp' ? (me?.handicap_index_pp ?? null) : (me?.handicap_index ?? data?.myHandicap ?? null)
  const hcpHistory: number[] = (data?.whsHistory ?? []).flatMap(d =>
    (courseType === 'pp') === !!d.is_pp ? [parseFloat(d.differential.toFixed(1))] : [],
  )
  ...
  const girPct = n
    ? Math.round(rounds.reduce((a, r) => a + (r.gir_total > 0 ? (r.gir / r.gir_total) * 100 : 0), 0) / n)
    : null
  const fwPct = n
    ? Math.round(rounds.reduce((a, r) => a + (r.fairways_total > 0 ? (r.fairways / r.fairways_total) * 100 : 0), 0) / n)
    : null
  ...
  const companions = Object.entries(playerMap)
    .map(([id, v]) => ({ id, ...v }))
    .sort((a, b) => b.rounds - a.rounds)
  ...
  const winPhrase =
    winRate >= 70
      ? '¡Eres el terror del campo!'
      : winRate >= 50
        ? 'Ganando más que perdiendo, no está mal.'
        : winRate >= 30
          ? 'Queda algo de margen de mejora... bastante.'
          : '¡Ni te rindas, ni te lo tomes tan en serio!'
  const lossPhrase =
    winRate >= 70
      ? 'Los demás te deben mucho dinero.'
      : winRate >= 50
        ? 'Al menos no eres el último... todavía.'
        : winRate >= 30
          ? 'El golf te está enseñando humildad gratis.'
  ...
  return (
    <div className="min-h-screen bg-[#f4f1e9] pb-28">
      ...
      <div className="px-[14px] pt-4 pb-4">
        ...
        {section === 'campos' && <CamposSection key={courseType} rounds={rounds} courseStats={courseStats} />}
      </div>
    </div>
  )
}
          ...
          <div className="flex items-end justify-between mb-2">
            <div>
              ...
              {trend != null && Math.abs(trend) >= 0.05 && (
                <span
                  className="inline-flex items-center gap-1 mt-1.5 px-2 py-0.5 rounded-full font-mono text-[9px] font-bold"
                  style={{
                    backgroundColor: trend < 0 ? '#1f8a5b33' : '#e8b75a26',
                    color: trend < 0 ? '#7dd3a8' : '#e8b75a',
                  }}
                >
              ...
              return [
                {
                  ...
                  sub: avgDelta != null ? `${avgDelta > 0 ? '+' : ''}${avgDelta} vs par` : '',
                  trend:
                    avgScore != null
                      ? kpiTrend(
                          rAvg(r => r.total),
                ...
                },
                {
                  ...
                  trend:
                    girPct != null
                      ? kpiTrend(
                          rAvg(r => (r.gir_total > 0 ? (r.gir / r.gir_total) * 100 : 0)),
                          girPct,
                          false,
                        )
                      : null,
                },
                {
                  ...
                  trend:
                    fwPct != null
                      ? kpiTrend(
                          rAvg(r => (r.fairways_total > 0 ? (r.fairways / r.fairways_total) * 100 : 0)),
                          fwPct,
                          false,
                        )
                      : null,
                  ...
                  <div className="flex items-baseline gap-1.5 mt-1">
                    ...
                    {s.trend && (
                      <span
                        ...
                        style={{ color: s.trend.good ? '#1f8a5b' : '#c6432d' }}
              ...
              <div key={s.label} className="flex items-center gap-3 mb-2">
                ...
                <div className="flex-1 h-5 rounded-full overflow-hidden" style={{ backgroundColor: '#f4f1e9' }}>
                  <div
                    ...
                    style={{
                      width: `${Math.round((s.count / totalHolesP) * 100)}%`,
                      backgroundColor: s.bg,
                      border: `1px solid ${s.color}22`,
                    }}
                  />
                </div>
                <p className="font-mono text-[11px] font-bold w-8 text-right" style={{ color: s.color }}>
                  {Math.round((s.count / totalHolesP) * 100)}%
                </p>
              </div>
            ))}
          </div>
              ...
              <div className="flex items-center justify-between px-4 py-3 border-b border-[#efebe1]">
                <div className="flex items-center gap-3">
                  <div
                    ...
                    style={{ backgroundColor: ps.par === 3 ? '#2a6fdb' : ps.par === 4 ? '#1f8a5b' : '#d4a24a' }}
                  >
                    P{ps.par}
                  </div>
                  <div>
                ...
                <div className="text-right">
                  ...
                  >
                    {ps.avg - ps.par > 0 ? '+' : ''}
                    {(ps.avg - ps.par).toFixed(2)} vs par
                  </p>
                </div>
              </div>
              {/* Distribution */}
...
function SocialSection({
  ...
  const sAvgScore = socialRounds.length
    ? Math.round(socialRounds.reduce((a, r) => a + r.total, 0) / socialRounds.length)
    : null
  const sAvgDelta = socialRounds.length
    ? Math.round(socialRounds.reduce((a, r) => a + (r.total - r.real_par), 0) / socialRounds.length)
    : null
  const sGirPct = socialRounds.length
    ? Math.round(
        socialRounds.reduce((a, r) => a + (r.gir_total > 0 ? (r.gir / r.gir_total) * 100 : 0), 0) / socialRounds.length,
      )
    : null
  const sFwPct = socialRounds.length
    ? Math.round(
        socialRounds.reduce((a, r) => a + (r.fairways_total > 0 ? (r.fairways / r.fairways_total) * 100 : 0), 0) /
          socialRounds.length,
      )
    : null
  const sPutts = socialRounds.length
    ? parseFloat((socialRounds.reduce((a, r) => a + r.putts, 0) / socialRounds.length).toFixed(1))
    : null
  const sPen = socialRounds.length
    ? parseFloat((socialRounds.reduce((a, r) => a + r.penalties, 0) / socialRounds.length).toFixed(1))
    : null
  const sBunkers = socialRounds.length
    ? parseFloat((socialRounds.reduce((a, r) => a + r.bunkers, 0) / socialRounds.length).toFixed(1))
    : null
              ...
              <div className="grid grid-cols-3 gap-2">
                {[
                  ...
                  <div key={d.label} className="rounded-[12px] py-2.5 text-center" style={{ backgroundColor: d.bg }}>
                    ...
                    <p className="font-mono text-[22px] font-black leading-none mt-0.5" style={{ color: d.color }}>
                      {d.count}
                    </p>
                  </div>
                ))}
...
// ── CAMPOS ───────────────────────────────────────────────────
function CamposSection({ rounds, courseStats }: { rounds: RoundStat[]; courseStats: CourseStat[] }) {
    ...
    <div className="space-y-2">
      {selectedCourse &&
        (() => {
          ...
          const holeNums = Array.from(new Set(cRounds.flatMap(r => r.scores.map(s => s.hole_number)))).sort(
            (a, b) => a - b,
          )
          ...
          const scoreChip = (strokes: number | null, par: number) => {
            ...
            return (
              <div
                className="mx-auto w-7 h-7 rounded-[6px] flex items-center justify-center font-mono text-[12px] font-black"
                style={{ backgroundColor: bg, color: tx }}
              >
                {strokes}
              </div>
            )
      ...
      {!selectedCourse && courseStats.length > 0
        ? courseStats.map(c => {
                ...
                <div className="flex items-start justify-between mb-2">
                  ...
                  <div className="text-right flex-shrink-0">
                    ...
                    {deltaBest !== null && (
                      <p
                        className="font-mono text-[10px] font-bold"
                        style={{ color: deltaBest <= 0 ? '#1f8a5b' : '#9b6e1a' }}
                      >
