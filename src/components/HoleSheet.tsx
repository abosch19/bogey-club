              ...
              <div className="flex gap-2">
                {scoreOpts.map(s => {
                  ...
                  return (
                    ...
                    >
                      <span className="font-mono font-black leading-none" style={{ fontSize: isSel ? 20 : 17 }}>
                        {s}
                      </span>
                      {isSel && (
                        <span className="font-mono text-[9px] leading-none opacity-70">
                          {d > 0 ? `+${d}` : d === 0 ? 'par' : `${d}`}
                        </span>
                      )}
                    </button>
              ...
              <div className="flex gap-2 flex-1">
                {[0, 1, 2, 3, 4].map(n => (
                  <button
                    ...
                    style={{
                      backgroundColor: sc.putts === n ? '#0e1a16' : '#f4f1e9',
                      color: sc.putts === n ? '#fff' : '#6b7a72',
                    }}
                  >
                    {n}
                  </button>
    ...
    <div className="bg-white rounded-[16px] border border-[#e5e0d4] overflow-hidden">
      <div className="flex items-center gap-2.5 px-3 pt-3 pb-2.5">
        ...
        <div className="flex gap-1.5 flex-1">
          {scoreOpts.map(s => {
            ...
            return (
              ...
              >
                <span className="font-mono font-black leading-none" style={{ fontSize: isSelected ? 18 : 16 }}>
                  {s}
                </span>
                {isStableford && pts !== null && (
                  <span
                    className="font-mono leading-none mt-0.5"
                    style={{ fontSize: 12, color: isSelected ? c.text : '#6b7a72', opacity: 0.8 }}
                  >
                    {pts}pt
                  </span>
                )}
        ...
        <div className="flex items-center gap-1.5">
          {colors && sc.strokes != null && (
            <div
              className="w-8 h-8 rounded-[8px] flex items-center justify-center font-mono text-[12px] font-black"
              style={{ backgroundColor: colors.bg, color: colors.text }}
            >
              {sc.strokes}
            </div>
          ...
          >
            <svg width="10" height="10" viewBox="0 0 24 24" fill="none">
              <path
                d={isExp ? 'M18 15l-6-6-6 6' : 'M6 9l6 6 6-6'}
                stroke={isExp ? '#fff' : '#6b7a72'}
                strokeWidth="2.5"
                strokeLinecap="round"
      ...
      <div className="flex items-center gap-2 px-3 pb-3 border-t border-[#efebe1] pt-2.5">
        {par > 3 && (
          <button
            type="button"
            onClick={() => set(p.id, 'fairway', !sc.fairway)}
        ...
        <button
          type="button"
          onClick={() => set(p.id, 'in_bunker', !sc.in_bunker)}
          className="flex-1 py-2.5 rounded-[10px] text-[12px] font-bold transition"
...
function HoleEntry({ roundId, holeNum, onChangeHole, onFinish }: HoleEntryProps) {
  ...
  const hole: Hole | null = holeRow
    ? {
        hole_number: holeNum,
        par: holeRow.par,
        stroke_index: holeRow.stroke_index,
        distance_m: holeRow.distance_m ?? null,
      }
    : null
  ...
  async function handleSave() {
    ...
    try {
      await withTimeout(
        saveHoleMut({ roundId: roundId as Id<'rounds'>, hole_number: holeNum, scores: scoresArray }),
        5000,
      )
  ...
  return (
    <div className="flex flex-col">
      ...
      >
        <div
          className="absolute right-[-20px] top-[-20px] w-[80px] h-[80px] rounded-full"
          style={{ backgroundColor: '#1f8a5b', opacity: 0.85 }}
        />
        <div className="flex items-center gap-4 relative">
          <Drawer.Title asChild>
            <div className="text-[52px] font-black text-white leading-none">{holeNum}</div>
          </Drawer.Title>
