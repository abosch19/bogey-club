...
export function ScoreMark({ strokes, delta, size = 22 }: { strokes: number; delta: number; size?: number }) {
  ...
  if (delta === 0) {
    return <div className="flex items-center justify-center mx-auto" style={{ width: size, height: size }}>{number}</div>
  }
    ...
    <div className="flex items-center justify-center mx-auto" style={outer}>
      {double ? (
        <div className="flex items-center justify-center"
          style={{ width: size - 6, height: size - 6, border: `1.5px solid ${color}`, borderRadius: isCircle ? 9999 : 2 }}>
          {number}
        </div>
      ) : number}
    </div>
